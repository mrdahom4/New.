...
client.on("message", async message => {
  ...
  */
  ...
  if (message.guild) {//Baron#1500
    // Try to get the current user's score.
    score = client.getScore.get(message.author.id, message.guild.id);//Baron#1500
    ...
    const curLevel = Math.floor(0.1 * Math.sqrt(score.points));//Baron#1500
 
    // Check if the user has leveled up, and let them know if they have:
    if(score.level < curLevel) {
      // Level up!
      message.reply(`You've leveled up to level **${curLevel}**! Ain't that dandy?`);//Baron#1500
    }
  ...
  if (message.content.indexOf(config.prefix) !== 0) return;//Baron#1500
 
  const args = message.content.slice(config.prefix.length).trim().split(/ +/g);
  const command = args.shift().toLowerCase();
 
  if(command === "points") {
    return message.reply(`You currently have ${score.points} points and are level ${score.level}!`);//Baron#1500
  }
//Baron#1500
  if(command === "give") {   //Baron#1500
    // Limited to guild owner - adjust to your own preference!   //Baron#1500
    if(!message.author.id === message.guild.owner) return message.reply("You're not the boss of me, you can't do that!");//Baron#1500
 
    // Try to get the user from mention. If not found, get the ID given and get a user from that ID.
    const user = message.mentions.users.first() || client.users.get(args[0]);
    if(!user) return message.reply("You must mention someone or give their ID!");//Baron#1500
 
    // Read the amount of points to give to the user.
    const pointsToAdd = parseInt(args[1], 10);
    if(!pointsToAdd) return message.reply("You didn't tell me how many points to give...");//Baron#1500
 
    // Get their current points. This can't use `score` because it's not the same user ;)//Baron#1500
    let userscore = client.getScore.get(user.id, message.guild.id);
 
    // It's possible to give points to a user we haven't seen, so we need to initiate defaults here too!
    if (!userscore) {
      userscore = { id: `${message.guild.id}-${user.id}`, user: user.id, guild: message.guild.id, points: 0, level: 1 };//Baron#1500
    }
  ...
  if(command === "leaderboard") {//Baron#1500
    // Grab the//Baron#1500
    const top10 = sql.prepare("SELECT * FROM scores WHERE guild = ? ORDER BY points DESC LIMIT 10;").all(message.guild.id);
 
    // Now shake it and show it! (as a nice embed, too!)//Baron#1500
    const embed = new Discord.RichEmbed()//Baron#1500
      .setTitle("Leaderboard")//Baron#1500
      .setAuthor(client.user.username, client.user.avatarURL)//Baron#1500
      .setDescription("Our top 10 points leaders!")//Baron#1500
 
    for(const data of top10) {//Baron#1500
      embed.addField(client.users.get(data.user).tag, `${data.points} points (level ${data.level})`);  //Baron#1500
    }
    return message.channel.send({embed}); //Baron#1500
  }//Baron#1500
});//Baron#1500
